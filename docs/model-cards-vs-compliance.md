...
GOPAL v1.3.0 evaluates **184 declared facts** about an AI system across the frameworks shipped to the [validation preview](https://principledevolution.ai/playground). A declared fact is one no tool can measure for you — whether a CE marking was affixed, whether logs are retained for ten years.
...
| | Facts | Share |
| --- | --- | --- |
| **Answerable from a model card** | 8 | 4% |
| **Partially informed by one** | 21 | 11% |
| **Untouched by one** | 155 | 84% |
| Total | 184 | |

A complete, well-written model card answers **8 of 184** outright and gives you material for 21 more. The remaining **155 (84%)** are not in a model card because they are not about the model. They are about the organisation around it: who signed the declaration of conformity, whether a notified body was involved, whether the deployer told the affected workers, whether logs are kept for six months.
...
| `documentation.model_card.completeness` | the card is the artefact being scored |
...
| `model.free_and_open_source` | `license` in the card's YAML metadata |
| `model.parameters_publicly_available` | the weights are either in the repo or they are not |
