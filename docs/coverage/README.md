...
Every policy now has a sibling test file, as [CONTRIBUTING.md](../../CONTRIBUTING.md) requires. Most of them also assert that the policy denies an empty input, which is the single test that catches the fail-open class described below.
