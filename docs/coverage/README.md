...
The gate exists because the convention alone was insufficient. 22 of the 96 files then counted as policies had no test at all, including both Article 5 prohibited-practice policies, which gate the practices the Act bans outright. An untested policy produces no signal when its default is inverted: changing `default allow := false` to `default allow := true` in the social-scoring policy, as an external reviewer demonstrated, left the suite reporting 604/604. A passing suite and a per-file count measure different properties, and the files without coverage were the prohibitions rather than the paperwork.
