...
allow if {
	...
	declarations.resolve(input, ["ai_system", "risk_category"]) == "high_risk"
...
allow if {
	...
	declarations.resolve(input, ["ai_system", "risk_category"]) == "other_systems"
	declarations.resolve(input, ["ai_system", "basic_requirements_met"])
}
...
excessive_risk_prohibited if {
	declarations.resolve(input, ["ai_system", "risk_category"]) == "excessive_risk"
	declarations.resolve(input, ["ai_system", "type"]) == "autonomous_weapons_system" # Example of a prohibited system
}
...
right_to_explanation := {"allow": true, "msg": "Right to explanation met."} if {
	declarations.resolve(input, ["rights", "explanation_provided"])
}
...
right_to_contest := {"allow": true, "msg": "Right to contest met."} if {
	declarations.resolve(input, ["rights", "contest_mechanism_available"])
}
...
right_to_human_review := {"allow": true, "msg": "Right to human review met."} if {
	declarations.resolve(input, ["rights", "human_review_available"])
}
...
algorithmic_impact_assessment := {"allow": true, "msg": "Algorithmic impact assessment conducted."} if {
	declarations.resolve(input, ["compliance", "algorithmic_impact_assessment_conducted"])
}
...
robustness_accuracy_reliability := {"allow": true, "msg": "Robustness, accuracy, and reliability ensured."} if {
	declarations.resolve(input, ["compliance", "robustness_ensured"])
	declarations.resolve(input, ["compliance", "accuracy_ensured"])
	declarations.resolve(input, ["compliance", "reliability_ensured"])
}
...
oversight_authority := {"allow": true, "msg": "Oversight authority requirements met."} if {
	declarations.resolve(input, ["compliance", "oversight_authority_engaged"])
}
