...
allow if {
	declarations.resolve(input, ["remote_pilot", "certificate_held"]) == true
	declarations.resolve(input, ["aircraft", "registered"]) == true
	declarations.resolve(input, ["operation", "altitude_ft"]) <= object.get(input, ["params", "max_altitude_ft"], 400)
	visual_conditions_met
	lighting_conditions_met
}

visual_conditions_met if {
	declarations.resolve(input, ["operation", "visual_line_of_sight"]) == true
}

visual_conditions_met if {
	declarations.resolve(input, ["operation", "visual_line_of_sight"]) == false
	declarations.resolve(input, ["operation", "bvlos_waiver_held"]) == true
}

lighting_conditions_met if {
	declarations.resolve(input, ["operation", "daylight_or_lit"]) == true
}

policy_metrics := {
	"remote_pilot_certificated": {
		"name": "Remote Pilot Certificate Held",
		"value": declarations.resolve(input, ["remote_pilot", "certificate_held"]),
		"control_passed": declarations.resolve(input, ["remote_pilot", "certificate_held"]) == true,
	},
	"aircraft_registered": {
		"name": "Aircraft Registered",
		"value": declarations.resolve(input, ["aircraft", "registered"]),
		"control_passed": declarations.resolve(input, ["aircraft", "registered"]) == true,
	},
	"altitude_within_limit": {
		"name": "Altitude Within 400ft Limit",
		"value": declarations.resolve(input, ["operation", "altitude_ft"]),
		"control_passed": declarations.resolve(input, ["operation", "altitude_ft"]) <= object.get(input, ["params", "max_altitude_ft"], 400),
	},
	"visual_conditions": {
