...
test_content_safety_eval_fails_true if {
	fair_lending.content_safety_eval_fails with input as {
		"metrics": {"content_safety": {"score": 0.80}},
...
test_content_safety_eval_fails_false if {
	not fair_lending.content_safety_eval_fails with input as {
		"metrics": {"content_safety": {"score": 0.90}},
		"params": {"content_safety_threshold": 0.85},
	}
}
...
test_risk_management_eval_fails_true if {
	fair_lending.risk_management_eval_fails with input as {
		"metrics": {"risk_management": {"score": 0.80}},
...
test_risk_management_eval_fails_false if {
	not fair_lending.risk_management_eval_fails with input as {
		"metrics": {"risk_management": {"score": 0.90}},
		"params": {"risk_management_threshold": 0.85},
	}
}
...
test_is_compliant_without_params_uses_default_thresholds if {
	fair_lending.is_compliant with input as {"metrics": {
		"fairness": {"score": 0.95},
		"content_safety": {"score": 0.90},
		"risk_management": {"score": 0.90},
	}}
...
test_single_low_score_denies_and_is_reported if {
	fair_lending.is_compliant == false with input as {"metrics": {
		"fairness": {"score": 0.10},
		"content_safety": {"score": 0.95},
		"risk_management": {"score": 0.95},
	}}
	fair_lending.failed_evaluations == ["fairness"] with input as {"metrics": {
		"fairness": {"score": 0.10},
		"content_safety": {"score": 0.95},
		"risk_management": {"score": 0.95},
	}}
...
test_missing_one_score_is_treated_as_a_failure if {
	...
	fair_lending.failed_evaluations == ["risk management"] with input as {"metrics": {
		"fairness": {"score": 0.95},
		"content_safety": {"score": 0.95},
	}}
}

# This report used to vanish entirely for a submission with no scores, because
# an undefined value inside the object made the whole object undefined. The
# submission that most needs a report produced none.
test_report_exists_and_says_null_for_unmeasured_scores if {
	report := fair_lending.compliance_report with input as {"params": {}}

	report.scores.fairness == null
	report.scores.content_safety == null
	report.scores.risk_management == null
