...
data_minimization_compliant if {
	every field in input.data_requested {
		is_necessary_for_purpose(field, declarations.resolve(input, ["request", "purpose"]))
	}
}
...
deny contains msg if {
	...
	superfluous_data := {field | some field in input.data_requested; not is_necessary_for_purpose(field, declarations.resolve(input, ["request", "purpose"]))}
	msg := sprintf("Data minimization violation: The following data fields are not necessary for the purpose '%v': %v", [declarations.resolve(input, ["request", "purpose"]), superfluous_data])
}
