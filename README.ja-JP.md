...
**Governance Open Policy Agent Library** — Rego で記述された [OPA](https://www.openpolicyagent.org/) ポリシーを厳選したコレクションです。EU AI Act、NIST AI RMF、航空安全標準、教育分野の FERPA / COPPA、銀行業の公正融資規則など、実際の AI ガバナンス要件をコードで表現しています。
...
3. **バージョン管理されている。** すべてのフレームワークは `v1/` (続いて `v2/` …) の配下に置かれ、明示的なセマンティック・バージョニング保証があります。詳細は [COMPATIBILITY.md](COMPATIBILITY.md) を参照してください。EU AI Act が改正されても、旧バージョンはそのまま残ります。
